...
class MarkMarkPosBuilder(LookupBuilder):
    ...
    def build(self):
        ...
        marksByClass = defaultdict(dict)  # markClassName -> { glyphName -> (0, anchor) }
        for glyphName, (markClassName, anchor) in self.marks.items():
            # 0 because each will be in a different subtable
            marksByClass[markClassName][glyphName] = (0, anchor)

        baseMarksByClass = defaultdict(lambda: defaultdict(dict))  # markClassName -> { glyphName -> {markClassName -> anchor} }
        for glyphName, markClassNameToAnchor in self.baseMarks.items():
            for markClassName, anchor in markClassNameToAnchor.items():
                baseMarksByClass[markClassName][glyphName][markClassName] = anchor
        ...
        for markClassName, baseMarks in baseMarksByClass.items():
            ...
            st.Format = 1
            st.ClassCount = 1  #len(markClasses)
